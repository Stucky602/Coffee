...
def draw_bean(D, base, kind, overlay=None):
    ...
    # roast-specific
        ...
        # burnt tips: darker, bigger char blooms at both ends
        paste_blob(cx,cy-ry*0.86,rx*0.55,(14,9,4),245,blur=rx*0.10)
        paste_blob(cx,cy+ry*0.86,rx*0.55,(14,9,4),245,blur=rx*0.10)
    elif overlay=='facing':
...
def gallery(fname, title, kind, base, cells):
    ...
    im=im.resize((W*2,H*2), Image.LANCZOS)  # 2x device scale for crisp text
    im.save(OUT/fname)
...
def strip(fname, kind, base, cells, per_row=4):
    ...
    im=im.resize((W*2,H*2),Image.LANCZOS); im.save(OUT/fname); print("wrote",fname,im.size)  # 2x device scale
